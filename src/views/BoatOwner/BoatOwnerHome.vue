      ...
      <div class="carousel-inner py-4">
        ...
        <div class="carousel-item active">
          <div class="container">
            <div class="row">
              <div
                v-for="(boatDto, index) in filteredBoats"
                :key="index"
                class="col-lg-4"
              >
...
export default {
  ...
  methods: {
    getOwnersBoats: function () {
      ...
      axios
        .post(
          process.env.VUE_APP_BACKEND_URL + "boats/findBoatsByOwnersUsername",
          this.user
        )
        .then((response) => {
    ...
    getImageUrl: function (index) {
      if (this.boatsLoaded == true) {
        ...
        this.entityImages.forEach((image) => {
          if (image.fileName === this.filteredBoats[index].images[0].url) {
    ...
    getFullAddress: function (index) {
      if (this.boatsLoaded == true)
        return (
          this.filteredBoats[index].addressDto.streetAndNum +
          ", " +
          this.filteredBoats[index].addressDto.city +
          ", " +
  ...
  computed: {
    filteredBoats: function () {
      var temp = this.boatDtos.filter((boat) => {
        return (
          boat.name.toUpperCase().match(this.searchName) &&
          boat.type.toUpperCase().match(this.searchType) &&
          boat.addressDto.streetAndNum
            .toUpperCase()
